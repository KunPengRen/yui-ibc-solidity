...
func (c *Coordinator) ApproveAndDepositTokenWithPrvKey(
	ctx context.Context,
	chain *Chain,
	senderIndex uint32,
	amount uint64,
	receiverPrvKey *ecdsa.PrivateKey,
) error {
	berforeBalance, err := chain.ERC20.BalanceOf(chain.CallOpts(ctx, senderIndex), chain.CallOpts(ctx, senderIndex).From)
	if err != nil {
		return err
	...
	beforeBankBalance, err := chain.ICS20Bank.BalanceOf(
		chain.CallOptsFromPrvKey(ctx, receiverPrvKey),
		chain.CallOptsFromPrvKey(ctx, receiverPrvKey).From,
		strings.ToLower(chain.ContractConfig.ERC20TokenAddress.String()))
	if err != nil {
		return err
	}
